package MentoringwithAhmet.interviewq;
...
//    Create the method to print the Dairy.(do it with the list, think about the reason why )
        public void PrintDairy () {
            ...
            List<String> shortway = new ArrayList<>(Arrays.asList(dairy)); //shortway
...
public  void DairyNameAndPrice() {
    Map<String, Double> listoftheDailyPrice = new HashMap<>();
    for (int i = 0; i<dairy.length; i++ ) {
        listoftheDailyPrice.put(dairy[i],dairyPrice[i]);
    }
    System.out.println(listoftheDailyPrice);
}
